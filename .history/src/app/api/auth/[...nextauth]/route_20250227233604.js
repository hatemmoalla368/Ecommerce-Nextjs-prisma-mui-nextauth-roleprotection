...
export const authOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        if (dbUser) {
          token.adresse = dbUser.adresse; // ✅ Fetch adresse for returning users
        }
      }
    ...
    async session({ session, token }) {
      ...
      session.user.adresse = token.adresse; // ✅ Include adresse in session
  ...
  pages: {
    signIn: "/login", // Redirect to custom login page (optional)
  },
